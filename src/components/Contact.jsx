    ...
    useEffect(()=>{
     async function getLandlord(){
      ...
      if (docSnap.exists()) {
        ...
        toast.error("couldnt get land lord data")
      }
     }
     getLandlord();
    },[userRef]);
  ...
  return (
    <>{landlord !== null && (
        ...
        <a href={`mailto:${landlord.email}?Subject=${listing.name}&body=${message}`}>
          ...
          focus:bg-blue-700 focus:bg-blue-700 focus:shadow-lg
          active:bg-blue-800 active:shadow-lg transition duration-150
          ease-in-out w-full text-center mb-6' type='button'>Send Message</button>
        </a>
        </div>
    )}</>
  )
